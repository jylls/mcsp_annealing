...
void greedy_merging(Partition &p_struct) {
    ...
    while (best_delta > 0) {
        ...
        best_delta = 0;
        double best_merge = 0;
        unsigned int best_i, best_j, last_i; // if initialize --> give very wrong results.... check what's going on! =100000; // last_i initialized to an impossible number
    ...
    while (best_delta > 0) {
        ...
        for (unsigned int i = 0; i < p_struct.n; i++){
            for (unsigned int j = i + 1; j < p_struct.n; j++){
                ...
                if ((depth == 0) || (i == last_i) || (j == last_i)) {
                    double unmerged_evidence = icc_evidence_vect(ci, p_struct) + icc_evidence_vect(cj, p_struct);
                    double merged_evidence = icc_evidence_vect(cij, p_struct);
                    delta_evidence = merged_evidence - unmerged_evidence;
                    __uint128_t identifier = (ONE << i) + (ONE << j);
                    calculated_evidence[identifier] = delta_evidence;           
                } else {
                ...
                if (delta_evidence > best_delta) {
                    best_delta = delta_evidence;
                    best_merge = icc_evidence_vect(cij, p_struct);
                    best_i = i;
                    best_j = j;
                    best_community = cij;
                }
